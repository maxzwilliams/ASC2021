...
\section*{Results}
%% Here I need to get some good results from my program.
We investigate three questions about convection in the inner core. Does the geometry matter? Does the convective behavour change between

\subsection*{How much does the geomtry matter}
Here I want to have a comparison of the convection patterns that I get from running the two codes with similar parameters. I will project the polar one back into cartesian coordinates for better veiwing. I will also have the energy graph showing that these streamfunction codes do not conserve energy very well, and so do not really work that well for this type of simulation.


\begin{figure}
	\centering
	\includegraphics{frog.png}
	\caption{Internally Heated Convection comparison between cartesian case (left) and polar case (right) $Pr=1, Ra=10^8$}
